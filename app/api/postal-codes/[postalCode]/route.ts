...
export async function GET(_request:Request,{params}:{params:Promise<{postalCode:string}>}){
 ...
 try{
  const response=await fetch(`${base.replace(/\/$/,'')}/public/store/postal-codes/${postalCode}`,{headers:{...await storeRequestIdentity(),'X-Store-Tenant':await getStoreTenant()},cache:'no-store',signal:AbortSignal.timeout(12000)});
